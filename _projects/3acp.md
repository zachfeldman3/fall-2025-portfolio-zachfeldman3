...
To validate the analytical results, we modeled the bolted region directly in ACP. The clamp load was applied through a representative washer diameter (≈0.75 in), and the through-thickness compressive stress in the foam core was evaluated under increasing preload. To simulate the load of the washer, we selected a refined mesh of .002m and used node selection to select the nodes on the mesh where the washer would be and used that selection to base the force off of.

## Node Selection on Surface Mesh
...
The boundary conditionswere defined to replicate the physical load path created by a preloaded fastener. Rather than applying two equal and opposite “crushing forces” on the top and bottom faces, the bottom surface and side faces were defined as fixed supports, and a downward force was applied at the top bearing region corresponding to the bolt head/washer contact area.
...
A baseline preload of 1000 N was applied and scaled upward to determine the maximum allowable clamp force before core crushing occurred. The ACP failure results showed that core crushing began at approximately ~1200 N maximum allowable preload based off of the safety factor generated by the composite failure tool. The Composite Failure Tool evaluates the likelihood of failure in each ply of a composite laminate by comparing the computed stress or strain state to material-specific failure criteria defined for the lamina.
...
To validate our composite modeling approach, I am performing three-point bending tests on carbon fiber coupons representing the various layups and sandwich configurations used in our chassis. Each specimen is loaded to failure under controlled conditions, allowing us to measure stiffness, peak load, and failure mode (e.g., fiber fracture, matrix cracking, core crushing, or delamination).

The same geometry, layup sequence, boundary conditions, and loading configuration are then recreated in ANSYS ACP to directly compare predicted deformation, stress distribution, and composite failure indices against experimental results. The goal is to determine whether the model accurately predicts both the failure load and the governing failure mechanism.

If discrepancies arise between experimental and simulated results, we systematically refine the ACP model. This may include adjusting orthotropic material properties (e.g., elastic moduli, shear moduli, strength allowables), updating failure criteria selection (e.g., Tsai-Wu vs. Hashin), refining ply orientations or thickness definitions, improving mesh density in high-gradient regions, or correcting boundary condition assumptions to better replicate real support conditions and load contact behavior.

This validation process is critical because it ensures that our composite simulations are not simply visually correct, but mechanically representative of real laminate behavior. By calibrating the model against physical testing, we increase confidence that ACP predictions of bolt pretension, bending loads, and structural performance accurately reflect real-world failure behavior, reducing design risk and preventing over- or under-conservative structural assumptions.
